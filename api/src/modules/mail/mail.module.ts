  ...
  providers: [
    {
      ...
      useFactory: (config: ConfigService<EnvironmentVariables, true>): MailService => {
        const logger = new Logger('MailModule');
        const host = config.get('SMTP_HOST', { infer: true });

        if (!host) {
          logger.warn('SMTP_HOST not set — using console mail service (dev only)');
          return new ConsoleMailService();
        }

        const smtp: SmtpConfig = {
          host,
          port: config.get('SMTP_PORT', { infer: true }) ?? DEFAULT_SMTP_PORT_TLS,
        ...
        logger.log(`Mail transport: SMTP ${smtp.host}:${smtp.port} (secure=${smtp.secure})`);
