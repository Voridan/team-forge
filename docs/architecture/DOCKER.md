...
| **livekit** | `livekit/livekit-server` | 7880, 7881, 3478 | 7880, 7881, 3478 | WebRTC SFU + built-in TURN/STUN |
...
- Ports: `7880:7880` (signaling), `7881:7881` (RTC), `3478:3478/udp` (built-in TURN), `50000-50060:50000-50060/udp` (media — small range for dev)
- Environment: `LIVEKIT_KEYS`
- Config file mount: `./docker/livekit/livekit-dev.yaml:/etc/livekit.yaml` (includes TURN config)

**nginx**
- Image: `nginx:alpine`
- Ports: `80:80`
- Volumes: `./docker/nginx/nginx.dev.conf:/etc/nginx/nginx.conf`
- Depends on: api, realtime, analytics
...
- Ports: `7880:7880`, `7881:7881`, `3478:3478/udp` (built-in TURN), `5349:5349/tcp` (TURNS), `50000-60000:50000-60000/udp` (full media port range)
- Health check: LiveKit exposes a health endpoint
- Restart: `unless-stopped`
- Resource limits: `cpus: '2.0'`, `memory: 1G`
- Config: LiveKit config YAML with TURN section enabled
...
# LiveKit (includes built-in TURN)
...
- LiveKit exposes its media and TURN ports directly (UDP traffic can't be proxied by Nginx)

```
┌── Frontend Network ──────────────────────────┐
│  Nginx ←→ API, Real-Time, Analytics          │
└──────────────────────────────────────────────┘
        │
┌── Backend Network ───────────────────────────┐
